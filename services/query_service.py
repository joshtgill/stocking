...
class QueryService:
    ...
    def start(self):
        for symbol in self.queryRequest.symbols:
            results = self.performQuery(symbol)

            resultsStr = ''
            for row in results:
                resultsStr += '{}\n'.format(row)

            fileService = FileService('data/{}_{}_to_{}_{}.txt'.format(symbol,
    ...
    def performQuery(self, symbol):
        ...
        while True:
            ...
            # Get data
            data = yf.Ticker(symbol).history(start=start, end=end, interval=self.queryRequest.interval)
